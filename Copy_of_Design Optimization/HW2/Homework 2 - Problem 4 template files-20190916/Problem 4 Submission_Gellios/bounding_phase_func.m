function [xbounds,function_calls] = bounding_phase_func(x0,step_size)
%%
%%%%%%%%%%%%%%%%%%%%
...
%   1) objF.m (objective function)
%%%%%%%%%%%%%%%%%%%%

%% Initialization (DO NOT EDIT)
f0 = objF(x0);          % Determine current objective function value
function_calls = 1;     % Account for first objective function call

%% Define step size direction 

x_left = x0 - abs(step_size);              %% COMPLETE THIS LINE
f_left = objF(x_left);              %% COMPLETE THIS LINE
function_calls = function_calls + 1;

x_right = x0 + abs(step_size);             %% COMPLETE THIS LINE
f_right = objF(x_right);             %% COMPLETE THIS LINE
function_calls = function_calls + 1;
...
if (step_size == 0)     % Solution is bounded by the step size
    ...
    while (convergence_flag == 0)
        ...
        xnew = xcurrent+2^k*step_size;            %% COMPLETE THIS LINE
        fnew = objF(xnew);            %% COMPLETE THIS LINE
        function_calls = function_calls + 1;
